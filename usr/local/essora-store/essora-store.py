...
class ProgressDialog(Gtk.Dialog):
    ...
    def update_progress(self, fraction, text=None):
        ...
        Update progress bar
        fraction: 0.0 to 1.0
        text: optional text for the bar
        """
    ...
    def set_pulsing(self, is_pulsing):
        """Activate/deactivate pulsing mode (indeterminate)"""
        if is_pulsing:
            GLib.timeout_add(100, self._pulse_callback)
    
    def _pulse_callback(self):
        """Callback for pulsing animation"""
...
class SimpleProgressDialog:
    """
    Simplified version that can be used without creating an instance manually
    Usage from ActivityManager with GLib.idle_add
    """
...
def _ensure_search_popover_css():
    """Aplica CSS específico del popover de resultados del buscador.
    Esto fuerza un fondo oscuro (282C34) incluso si el theme no define bien popovers/listas.
    """
...
class BackendPage(Gtk.Box):
    ...
    def refresh_repos(self):
        """Refresh repositories list with enable/disable switches"""
        if self.pkg_type == "deb":
    ...
    def _read_apt_repositories(self):
        """Lee todos los repositorios APT de sources.list y sources.list.d/"""
        import glob
        repos = []
    ...
    def _parse_sources_file(self, filepath):
        """Parse a sources.list file and extract repository entries"""
        repos = []
    ...
    def _create_repo_row(self, repo):
        """Crea una row GTK para un repositorio con switch"""
        row = Gtk.ListBoxRow()
        row.set_selectable(False)
        row.set_activatable(False)
    ...
    def _on_repo_switch_toggled(self, switch, state, repo):
        """Callback cuando se activa/desactiva un switch de repositorio"""
        import subprocess
    ...
    def _ensure_backend_search_popover(self):
        """Crea el popover de búsqueda para este backend si no existe"""
    ...
    def _on_backend_search_activate(self, entry):
        """Maneja la búsqueda cuando se presiona Enter en el campo de búsqueda del backend"""
    ...
    def _set_backend_search_results(self, apps):
        """Actualiza los resultados de búsqueda en el popover del backend"""
    ...
    def _run_backend_search(self, query):
        """Ejecuta la búsqueda filtrando solo por el tipo de paquete de este backend"""
    ...
    def _on_backend_search_row_activated(self, listbox, row):
        """Maneja el clic en un resultado de búsqueda del backend"""
...
class HomePage(Gtk.Box):
    ...
    def _load_flatpak_apps(self):
        """Carga apps Flatpak destacadas desde /usr/local/essora-store/flatpak.txt usando icon-cache.json."""
...
class EssoraStoreWindow(Gtk.Window):
    ...
    def _on_fix_broken_clicked(self, *args):
        """Launches essora-fix-broken.py — standalone GTK repair tool.
        #editado por josejp2424 — lanzado igual que el About dialog,
        sin pkexec porque el store ya corre como root.
        """
    ...
    def _on_about_clicked(self, *args):
        """Ejecuta el script essora_about_dialog.py"""
    ...
    def update_progress_text(self, text):
        """Actualiza el texto de la consola del diálogo"""
...
def check_essora_os():
    """Verifica que el sistema sea Essora Linux leyendo /usr/lib/os-release"""
...
class SingleInstance:
    """Asegura que solo haya una instancia de Essora Store corriendo.
    
    Usa un socket Unix para detectar si ya hay otra instancia.
    Si hay otra instancia, la trae al frente y termina.
    """
    ...
    def create_lock(self):
        """Crea el socket lock para esta instancia."""
        try:
    ...
    def _listen_for_show(self):
        """Escucha comandos de otras instancias."""
        while True:
            try:
